...
for i in range(no_blobs+1)
    x, y = np.random.default_rng().multivariate_normal(np.array([x_mid[i], y_mid[i]]), cov) #should have mean values as brightness, need to work out positions - maybe better to do each dimension separately?
